...
const ListSection = ({ transactions }: ListSectionProps) => {
  return (
...
const TransactionItem = ({
  ...
  const { icon: TransactionIcon, representativeColor, label } = categoryConfig!;
  ...
  return (
    <Card key={id} className="flex flex-col gap-2 bg-transparent ring-0">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-[12px] ${amountColor}`}>{formattedAmount}</span>
      </div>
